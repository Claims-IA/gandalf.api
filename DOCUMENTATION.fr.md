...
> **Valeurs de `matching_type` :** `first` (la première règle qui correspond gagne) · `scoring_sum` (somme de toutes les règles qui correspondent) · `scoring_max` (valeur la plus élevée parmi les règles qui correspondent) · `scoring_min` (valeur la plus basse parmi les règles qui correspondent) · `scoring_count` (nombre de règles qui correspondent)
...
| Opérateur        | Type d'entrée | Description                                              |
| ---------------- | ------------- | -------------------------------------------------------- |
| `$any`           | -             | N'importe quelle valeur ou null                          |
| `$between_excl`  | betweenString | Entre deux valeurs `min < x < max` (format : `min;max`)  |
...
## Flux du moteur de décision
	...
	2. Scoring::check($id, $values, $appId, $showMeta)
	   ...
	   d. Pour chaque Règle dans la variante (dans l'ordre) :
	      ...
	      Déterminer si TOUTES les conditions ont correspondu (logique ET) :
	        - Table premier-match (matching_type: 'first') : si tout correspond ET final_decision est encore null
	                              → définir final_decision = rule.than
	                              → mettre à jour title/description depuis la règle
	        - Table scoring_sum (matching_type: 'scoring_sum') : si tout correspond → ajouter rule.than (float) au total cumulé
	        - Table scoring_max (matching_type: 'scoring_max') : si tout correspond → conserver max(final_decision, rule.than)
	        - Table scoring_min (matching_type: 'scoring_min') : si tout correspond → conserver min(final_decision, rule.than)
	        - Table scoring_count (matching_type: 'scoring_count') : si tout correspond → incrémenter le compteur de 1
	...
	Exemple — Résultat d'une table scoring_sum :
	...
	Exemple — Résultat d'une table scoring_max :
	  Règles : "Bonne IP" (+10), "Banque de confiance" (+10), "Faible rotation" (+60)
	  Si les trois correspondent : final_decision = max(10, 10, 60) = 60
	  Si seules les deux premières correspondent : final_decision = max(10, 10) = 10
	  Si rien ne correspond : final_decision = default_decision (ex. 30)
	
	Exemple — Résultat d'une table scoring_min :
	  Règles : "Bonne IP" (+10), "Banque de confiance" (+10), "Faible rotation" (+60)
	  Si les trois correspondent : final_decision = min(10, 10, 60) = 10
	  Si seulement la première et la troisième correspondent : final_decision = min(10, 60) = 10
	  Si rien ne correspond : final_decision = default_decision (ex. 30)

	Exemple — Résultat d'une table scoring_count :
	  Règles : "Bonne IP" (+10), "Banque de confiance" (+10), "Faible rotation" (+60)
	  Si les trois correspondent : final_decision = 3
	  Si seules les deux premières correspondent : final_decision = 2
	  Si rien ne correspond : final_decision = default_decision (ex. 0)

	Exemple — Résultat d'une table premier-match (matching_type: 'first') :
